...
def print_time(t):
    ...
    print(t.days < 0 and '-' or '+', '{}h {}m {}s'.format(hours, minutes, seconds))
...
def to_delta(op, time):
    ...
    try: date_time = datetime.strptime(time, time_format)
    except ValueError:
...
calc_string = reduce(operator.concat, sys.argv[1:])
splitted_string = re.split(r'([+,-])', calc_string)
